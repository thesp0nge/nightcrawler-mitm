...
class CommandInjectionScanner(ActiveScanner):
    ...
    async def run(
        ...
        """
        Attempts basic command injection payloads.
        """
        payloads = self.addon_instance.cmd_injection_payloads
        if not payloads:
            return
        ...
        self.logger.debug(f"[Cmd Injection Scan] Starting for {url} with {len(payloads)} payloads.")
            ...
            for payload in payloads:
                ...
                try:
                    ...
                    if "sleep" in payload.lower() and duration > 4.5:
                        ...
                        try:
                            ...
                            await http_client.request(
                                ...
                                headers=request_headers, timeout=10.0
                            )
                            ...
                            elif v_duration > 4.5: continue
                        except Exception: pass

                        self.addon_instance._log_finding(
                            level="ERROR", finding_type="Command Injection? (Time-Based)",
                            url=url, detail=f"{payload_info_detail}{detail_suffix}",
                            evidence=payload_info_evidence, confidence=verified_confidence,
                        )
                        break

                    if response.text and "1787569" in response.text and "1337" not in response.text:
                        self.addon_instance._log_finding(
                            level="ERROR", finding_type="Command Injection (Verified Math)",
                            url=url, detail=f"Mathematical payload executed: 1337*1337 -> 1787569",
                            evidence={"param": param_name, "payload": payload, "output_snippet": "1787569"},
                            confidence="HIGH",
                        )
                        break
                    ...
                    if response.text and ("uid=0(root)" in response.text):
                        self.addon_instance._log_finding(
                            level="ERROR", finding_type="Command Injection? (Legacy Output)",
                            url=url, detail=f"Found suspicious output: {response.text[:50]}...",
                            evidence={"param": param_name, "snippet": response.text[:100]},
                            confidence="MEDIUM",
                        )
                        break
                except Exception as e:
                    self.logger.debug(f"[Cmd Injection Scan] Exception: {e} ({payload_info_detail})")
