...
async def scan_open_redirect(
    ...
    """
    Attempts to detect Open Redirect vulnerabilities by injecting an arbitrary URL
    into parameters that appear to accept URLs and checking for redirects.
    """
    ...
    logger.debug(f"[Open Redirect Scan] Starting for {url}")
    ...
    for param_name in params_to_fuzz:
        ...
        # Simple check: does the original value look like a URL?
        # This is a heuristic to reduce false positives and target likely candidates.
        if "http://" in original_value or "https://" in original_value:
            current_params = original_params.copy()
            current_data = original_data.copy()

            if is_param_in_query:
                current_params[param_name] = test_redirect_url
            else:
                current_data[param_name] = test_redirect_url
            
            # Ensure headers are cleaned up for the request
            request_headers = {
                k: v
            ...
            try:
                response = await http_client.request(
                    ...
                    follow_redirects=False # Crucial: we want to inspect the redirect response
                )
                ...
                if response.is_redirect:
                    ...
                    if location_header and test_redirect_url in location_header:
                        addon_instance._log_finding(
                            ...
                            url=url,
                            detail=f"Parameter '{param_name}' redirects to '{location_header}' (injected: '{test_redirect_url}')",
                            evidence={
                                "param": param_name,
                ...
                logger.debug(f"[Open Redirect Scan] Exception: {e} ({param_name})")
